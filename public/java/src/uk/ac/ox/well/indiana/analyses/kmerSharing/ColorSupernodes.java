...
public class ColorSupernodes extends Sketch {
    ...
    private void loadRelatedSequences() {
        ...
        for (Map<String, String> te : tr) {
            String supernode = SequenceUtils.getAlphanumericallyLowestOrientation(te.get("superNode"));

            Set<String> genesInSupernode = new HashSet<String>();

            for (int i = 0; i <= supernode.length() - KMER_SIZE; i++) {
                String fw = SequenceUtils.getAlphanumericallyLowestOrientation(supernode.substring(i, i + KMER_SIZE));
    ...
    public void setup() {
        ...
        for (String supernode : relatedSequences) {
            ...
            for (int j = 0; j <= supernode.length() - KMER_SIZE; j++) {
                String fw = SequenceUtils.getAlphanumericallyLowestOrientation(supernode.substring(j, j + KMER_SIZE));
