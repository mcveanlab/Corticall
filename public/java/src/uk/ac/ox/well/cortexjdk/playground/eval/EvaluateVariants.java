...
public class EvaluateVariants extends Module {
    ...
    public File TRUTH_TABLE;

    @Argument(fullName="calls_links", shortName="cl", doc="Calls (with links)")
    public File CALLS_LINKS;

    @Argument(fullName="calls_no_links", shortName="cnl", doc="Calls (no links)")
    public File CALLS_NO_LINKS;
    ...
    private void evaluateCalls(File truthTableF, File callsF, File novelsF) {
        ...
        for (int i = 0; i <= 251; i++) {
            if (i != 84) {
                File truthTable = new File(truthTableF.getAbsolutePath().replaceAll("child0", "child" + i));
                File callsFile = new File(callsF.getAbsolutePath().replaceAll("child0", "child" + i));
                File novelsFile = new File(novelsF.getAbsolutePath().replaceAll("child0", "child" + i));
                ...
                Map<GeneratedVariant, Set<VariantContext>> cvs = clusterCalledVariants(vcs, gvs);
                ...
                for (GeneratedVariant gv : cvs.keySet()) {
                    if (!gv.type.equals("rejected")) {
                        ...
                        for (VariantContext v : cvs.get(gv)) {
                            ...
                            if (Math.abs(gv.start.getStart() - v.getStart()) < 15) { val.closeToStart = true; }
                            if (Math.abs(gv.stop.getStart() - v.getStart() - v.getAltAlleleWithHighestAlleleCount().length()) < 15) { val.closeToStop = true; }
        ...
        columns.add(String.format("%-35s", "type"));
        ...
        for (String type : simTable.keySet()) {
            ...
            out.println(String.format("%-35s", type) + "\t" + Joiner.on("\t").join(row) + "\t" + String.format("%.2f", f1));
        }
    }
    ...
    private boolean isSameType(GeneratedVariant gv, VariantContext v) {
        if (gv.type.equalsIgnoreCase("SNV") && v.isSNP()) { return true; }
        ...
        if (gv.type.equalsIgnoreCase("MNP") && (v.isMNP() || v.isIndel())) { return true; }
        if (gv.type.equalsIgnoreCase("NAHR-INS") && v.isSymbolic()) { return true; }
    ...
    private boolean isApproxSameLocus(GeneratedVariant gv, VariantContext v, int seqIndex) {
        return (gv.seqIndex + 1 == seqIndex) || (Math.abs(gv.posIndex - v.getStart()) < 10000);
    }
