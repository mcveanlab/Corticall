...
import uk.ac.ox.well.cortexjdk.utils.containers.DataFrame;
...
public class CortexUtils {
    ...
    public static boolean isNovelKmer(CortexRecord cr, int childColor, Set<Integer> parentColors) {
        if (cr == null) {
            throw new CortexJDKException("Unable to test for novelty on a null Cortex record");
        }
    ...
    public static boolean isNovelKmer(CortexGraph cg, String kmer, int childColor) {
        ...
        if (cr == null) {
            throw new CortexJDKException("Unable to test for novelty on a kmer that's not in the graph: '" + kmer + "'");
        }
        ...
        for (int c : colors) {
            for (int i = 0; i < 2; i++) {
                ...
                for (String adjKmer : adjKmers.get(c)) {
                    ...
                    if (edgesAreForward) {
                        if (!hasExpectedOverlap(cv, av)) { throw new CortexJDKException("Kmers '" + cv.getKmer() + "' and '" + av.getKmer() + "' did not have expected overlap"); }

                        dfs.addEdge(cv, av, aen);
                    } else {
                        if (!hasExpectedOverlap(av, cv)) { throw new CortexJDKException("Kmers '" + cv.getKmer() + "' and '" + av.getKmer() + "' did not have expected overlap"); }
            ...
            while (nextKmers.size() > 0 && junctionsUsed < junctions.length()) {
                if (nextKmers.size() == 1) {
                    ...
                    if (nextKmers.contains(expectedNextKmer)) {
                        ...
                        throw new CortexJDKException("Junction record specified a navigation that conflicted with the graph: " + junctions + " " + nbase + " " + expectedNextKmer + " " + nextKmers);
                    }
            ...
            while (prevKmers.size() > 0 && junctionsUsed < junctions.length()) {
                if (prevKmers.size() == 1) {
                    ...
                    if (prevKmers.contains(expectedPrevKmer)) {
                        ...
                        throw new CortexJDKException("Junction record specified a navigation that conflicted with the graph: " + junctions + " " + pbase + " " + expectedPrevKmer + " " + prevKmers);
                    }
